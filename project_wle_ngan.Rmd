---
title: "Project Weight Lifting Exercise"
author: "Le Duy Ngan"
...
```
Step 2. Partition data set to use 10-fold CV later
```{r}
partition <- createFolds(y = ds$classe, k = 10)
```

Following are myPreProcess steps used to select features. Here the first partition is used for illustration.

Step 3. Pick first partition
```{r}
valid.set.1 <- ds[partition$Fold01, ]
train.set.1 <- ds[-partition$Fold01, ]
```

Step 4. myPreProcess stage has the following steps:
...
5. Train random forest with parameters ntree=100
```{r}
...
* Pre-process each pair (train, valid) as what we have done for the first pair (train.set.1, valid.set.1)
* Run random forest for each pair and get accuracy
* Get mean accuracy

Here are details.

* Function to pre-process each pair (same as illustrated above)
```{r}
